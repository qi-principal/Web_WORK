# 网站审核页面组件
<template>
  <div class="website-review-container">
...
<script>
import { getPendingWebsiteList, approveWebsite, rejectWebsite } from '@/api/admin'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getPendingWebsiteList(this.listQuery).then(response => {
    ...
    handleApprove(row) {
      this.$confirm('确认通过该网站吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        approveWebsite(row.id).then(() => {
          this.$message({
            type: 'success',
            message: '审核通过成功'
          })
    ...
    submitReject() {
      this.$refs['rejectForm'].validate((valid) => {
        if (valid) {
          rejectWebsite(this.rejectForm.id, this.rejectForm.reason).then(() => {
